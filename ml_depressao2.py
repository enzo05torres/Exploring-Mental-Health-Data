...
# %%
#city_cond = df["City"].value_counts()
#df["City"] = df["City"].apply(lambda x: x if city_cond[x] > 100 else "Kalyan")

# %%
#city_cond2 = df2["City"].value_counts()
#df2["City"] = df2["City"].apply(lambda x: x if city_cond2[x] > 100 else "Kalyan")
...
# %%
### Graficos do total de pessoas com depressão
...
ax = contagem.plot(kind='bar', figsize=(8,6), color=['salmon', 'skyblue'])
...
plt.title('Estudantes: Depressão por Duração do Sono', fontsize=14)
...
plt.title('Trabalhadores: Depressão por Duração do Sono', fontsize=14)
...
plt.title('Pessoas: Depressão por Duração do Sono', fontsize=14)
plt.xlabel('Quantidade de Pessoas', fontsize=12)
plt.ylabel('Duração do Sono', fontsize=12)
plt.legend(title='Depressão', labels=["Não",'Sim'])
...
test_predictions = gbc.predict(df_final2)
